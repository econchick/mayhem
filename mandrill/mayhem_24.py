...
"""
Working with threaded blocking code - incorrect approach #2, showing
not being threadsafe.
...
async def handle_message(pubsub_msg):
    ...
    # Never actually get to here
    logging.info(f'handle_message - current thread: {thread.name}')
...
async def get_from_queue():
    """Get item from shared queue to pass off to handle_message."""
    while True:
        thread = threading.current_thread()
        # Never actually get to here
